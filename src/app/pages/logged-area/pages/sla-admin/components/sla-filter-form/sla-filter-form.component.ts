...
@Component({
  selector: 'app-sla-filter-form',
  templateUrl: 'sla-filter-form.component.html',
  styleUrls: ['sla-filter-form.component.scss'],
  standalone: true,
  imports: [ReactiveFormsModule, TranslateModule, SelectBoxModule],
})
export class SlaFilterFormComponent implements OnChanges, OnInit, OnDestroy {
  private destroyRef = inject(DestroyRef);

  @Input()
  filters: SlaFilterLike;
  @Output()
  readonly filtersChange = new EventEmitter<SlaFilterLike>();
    ...
    this.negotiationTypeDataSource =
      new CustomRawDataSource<NegotiationTypeModel>({
        load: () => {
          ...
          if (!value) {
            return this.negotiationTypesService.list();
          }
